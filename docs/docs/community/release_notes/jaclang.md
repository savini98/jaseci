...
- **Fix: `jac run` Crash on Decorated Python Classes**: Fixed a bug where running Python files containing decorated classes (e.g., `@torch.compile()`) via `jac run` would crash with `list index out of range`. Class decorator nodes were not included in the AST child list, so they were never traversed and their generated Python AST remained empty. The same issue also affected f-string format specifiers (e.g., `f"{value:.2f}"`). Both are fixed by including decorator and format-spec nodes in the child list for proper traversal.
- **Fix: `super().__init__()` Calls in Python Files**: Fixed `super().__init__()` calls in Python files parsed by `jac run`. Jac internally converts `__init__` to `init`, but `super()` calls require the original dunder name. The fix restores `__init__` when the call target is `super()`.
