...
## Computed fields: `postinit`

The sanctioned pattern for a field whose value derives from other fields. The `postinit` modifier excludes it from the constructor; `def postinit` runs after the auto-generated init and must assign it:
...
(The `postinit` line currently draws a spurious W1051 warning; `jac check` still passes.)
...
- **Python reserved words can't name fields, even backtick-escaped** - `` has `class: str; `` fails `jac check` with E0067 (the generated Python needs the name as a real identifier). Pick a non-reserved name (`kind`, `cls`). Jac-only keywords that aren't Python keywords (`` `visit ``, `` `node ``) escape fine. See `jac-core-cheatsheet`.
