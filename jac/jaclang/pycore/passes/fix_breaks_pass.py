...
class FixDynBreaksPass(UniPass):
    ...
    def check_method_call(self, node: uni.ExprStmt) -> tuple | None:
        ...
        # Extract method name
        method_name = None
        if isinstance(call.target.right, uni.Name) or hasattr(
            call.target.right, "value"
        ):
    ...
    def exit_if_stmt(self, node: uni.IfStmt) -> None:
        ...
        has_dyn_cf_break = getattr(node, "has_break_dyn_cf", False) or getattr(
            node, "has_dynamo_graph_break", False
        )
            ...
            if a_call is not None and b_call is not None:
                ...
                if a_method == b_method and set(a_kwargs.keys()) == set(
                    ...
                    if len(a_args) >= 2 and len(b_args) >= 2:
                        ...
                        if isinstance(
                            a_args[0], (uni.String, uni.MultiString)
                        ) and isinstance(b_args[0], (uni.String, uni.MultiString)):
                            a_str = (
                                a_args[0].value
                                if isinstance(a_args[0], uni.String)
                                else a_args[0].strings[0].value
                            )
                            b_str = (
                                b_args[0].value
                                if isinstance(b_args[0], uni.String)
                                else b_args[0].strings[0].value
                            )
                            first_arg_same = a_str == b_str

                        if first_arg_same:
                            # Extract expressions into temporary variables
                            temp_assigns, cond_ref, true_ref, false_ref = (
                            ...
                            for k, v in a_kwargs.items():
                                ...
                                kwargs_nodes.append(
                                    uni.KWPair(key_node, v, [key_node, v])
                                )
                            ...
                            original_call_target = (
                                a0.expr.target
                                if isinstance(a0.expr, uni.FuncCall)
                                else None
                            )
