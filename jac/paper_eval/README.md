...
- **`--graphmend-scope` cannot reach Hub remote code.** transformers loads it
  with `spec_from_file_location` + `exec_module`, which bypasses
  `sys.meta_path`, so jaclang's importer never sees the module. The runner
  therefore compiles that module's source through the same GraphMend pipeline
  and re-execs the result (`_transform_remote_module`). The transformation is
  identical; only the delivery path differs. This is a genuine limitation of
  the `--graphmend-scope` mechanism, not of the transformation.
